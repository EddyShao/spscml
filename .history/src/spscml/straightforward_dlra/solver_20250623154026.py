...
class Solver(eqx.Module):
    ...
    def K_step_single_species_RHS(self, K, grid, args):
        ...
        term1 =  (nu + flux_out) * (V @ M.T)
        print(f"Term1 shape: {term1.shape}, V shape: {V.shape}, M shape: {M.shape}")
        collision = (nu + flux_out) * (V @ M.T) -  nu * K @ (V @ V.T) 
        print(f"Collision term shape: {collision.shape}, K shape: {K.shape}, V shape: {V.shape}")
