...
class Solver(eqx.Module):
    ...
    def S_step_single_species_RHS(self, S, grid, args):
        ...
        E_plus = jnp.where(EZA > 0, EZA, 0.0)
        E_minus = jnp.where(EZA < 0, EZA, 0.0)
        print(E_plus.shape, E_minus.shape)
        E_Xp = X.T @ (jnp.diag(E_plus) @ X.T).T
        E_Xm = X.T @ (jnp.diag(E_minus) @ X.T).T
        print('E_Xp.shape: ', E_Xp.shape, 'E_Xm.shape: ', E_Xm.shape)
        ...
        M = maxwellian(grid, A, n)
        collision_rhs = nu * (V @ M.T) - nu * (V @ V.T) @ S  
        
        return v_term + E_term + collision_rhs
