...
class Solver(eqx.Module):
    ...
    def step_K(self, t, ys, args):
        ...
        def step_Ks_with_E_RHS(Ks):
            ...
            boundary_conditions = args['bcs']
            E_field = solve_poisson_KV({'electron': K_e, 'ion': K_i}, self.grids, boundary_conditions,self.plasma)
    ...
    def K_step_single_species_RHS(self, K, grid, args):
        ...
        E = solve_poisson_KV(K, V, self.plasma, self.grids, args['bcs'], flux_out)
        ...
        E_flux = (Dp_mat[:, :, None] * E_plus.T + Dm_mat[:, :, None] * E_minus.T)
        E_df_dv_term = (self.plasma.omega_c_tau * Z / A) * jnp.einsum("ijl,jl->il", E_flux, K)
        ...
        f = (K.T @ V).T  # shape (Nx, Nv)
        n = jnp.sum(f, axis=1) * dv
        M = maxwellian(grid, A, n)
        collision = nu * ((V @ (M.T * dv)).T - K)
    ...
    def S_step_single_species_RHS(self, S, grid, args):
        ...
        E = solve_poisson_KV((X.T @ S).T, V, self.plasma, self.grids, flux_out)  # shape (Nx,)
        E = E[:, None]
        ...
        f = (X.T @ S @ V).T  # shape (Nx, Nv)
        n = jnp.sum(f, axis=1) * dv
        M = maxwellian(grid, A, n)
        bgk_rhs = nu * (X.T @ (M @ V.T) * dv - S)
        
        return v_term + E_term + bgk_rhs
    ...
    def L_step_single_species_RHS(self, L, grid, args):
        ...
        E = solve_poisson_KV(K, V, self.plasma, self.grids, flux_out)  # shape (Nx,)
        E = E[:, None]
