...
class Solver():
    ...
    def implicit_euler_step(self, y, Vp, T, n, dt, sheath_solve):
        ...
        def residual(Q, V):
            ...
            Q = Q * ureg.coulombs
            V = V * ureg.volts
            Ip = sheath_solve(V, T, n) * ureg.ampere
            coef =  Lp / (L - Lp)
            V_RP = (V - coef * Ip * (-Q / C - R * Ip)) / (1 - coef)
            res_Q = Q - (Q_n + dt * (Ip + V / R))
            res_Vp = - Ip + Ip_n + dt * (- Q_n / C - R * Ip - V_RP ) / (L - Lp)

            return jnp.array([res_Q.magnitude, res_Vp.magnitude])
        ...
        root = optx.root_find(residual, self.rootfinder, y_guess, max_steps=10, throw=False,
                              options={'lower': jnp.array([0.0, -1.0e6]),
                                       'upper': jnp.array([1.0e6, 1.0e6])})
        if not root.converged:
            raise RuntimeError(f"Implicit Euler step did not converge: {root}")
        Q_new, V_new = root.value
        Ip_new = sheath_solve(V_new, T, n) 
        # do one step of the implicit Euler method
        y_new = jnp.array([Q_new, Ip_new.magnitude])
