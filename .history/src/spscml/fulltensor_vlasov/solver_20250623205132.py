...
class Solver(eqx.Module):
    ...
    def vlasov_rhs(self, fs, boundary_conditions, f0):
        ...
        print("Electron RHS shape: ", electron_rhs.shape)
    ...
    def vlasov_fp_single_species_rhs(self, f, E, A, Z, grid, bcs, nu):
        ...
        n_s = zeroth_moment(f, grid)[:, None]  # shape (Nx, 1)
        M_s = maxwellian(grid, A, n_s)
        print("M_s shape: ", M_s.shape)
        print("f shape: ", f.shape)
        print("nu shape: ", nu.type)
        Q = nu * (M_s - f)
        print("Q shape: ", Q.shape)
        print("vdfdx shape: ", vdfdx.shape)
        print("Edfdv shape: ", Edfdv.shape)
