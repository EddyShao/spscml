...
class Solver(eqx.Module):
    ...
    def vlasov_fp_single_species_rhs(self, f, E, A, Z, grid, bcs, nu):
        ...
        G = lambda left, right: jnp.where(E * (Z/A) > 0, left * E * (Z/A), right * E * (Z/A))
        Edfdv = slope_limited_flux_divergence(f_bc_v, 'minmod',
                                             lambda left, right: jnp.where(grid.vT > 0, left * E, right * E),
                                             grid.dv,
                                             axis=1)

        # HACKATHON: implement BGK collision term
        M_s = maxwellian(grid, A, n, T=1.0)
        Q = nu * (M_s - f)
        omega_c_tau = self.plasma.omega_c_tau
